<template>
    <section class="cntnt__zone cntnt__intro">
        <div class="cntnt__zone-inner cntnt__intro-inner">
            <div class="cntnt__intro-power">
                Powered by
                <img src="@/assets/images/elrond-logo.png" alt="Elrond Logo" />
            </div>
            <h2 class="cntnt__intro-title">A trading card game project</h2>
            <p class="cntnt__intro-txt">Collect cards, Trade, Battle</p>
            <a href="https://elrondplace.io/launchpad" target="_blank" class="cntnt__zone-btn cntnt__intro-btn">Buy a pack now</a>
        </div>
    </section>
</template>

<script>
export default {

}
</script>